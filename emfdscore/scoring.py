...
def score_emfd_all_sent(doc):
    ...
    moral_words = [emfd[token] for token in words if token in emfd.keys()]
    ...
    if len(moral_words) != 0:
        ...
        nonmoral_words = len(words) - len(moral_words)
        try:
...
def score_emfd_single_sent(doc):
    ...
    # Collect e-MFD data for all moral words in document
    moral_words = [
        emfd_single_sent[token] for token in words if token in emfd_single_sent.keys()
    ]
...
def score_emfd_all_vice_virtue(doc):
    ...
    # Collect e-MFD data for all moral words in document
    moral_words = [
        emfd_all_vice_virtue[token]
        for token in words
        if token in emfd_all_vice_virtue.keys()
    ]
...
def score_emfd_single_vice_virtue(doc):
    ...
    # Collect e-MFD data for all moral words in document
    moral_words = [
        emfd_single_vice_virtue[token]
        for token in words
        if token in emfd_single_vice_virtue.keys()
    ]
...
def score_mfd(doc):
    ...
    if moral_words != 0:
        ...
        nonmoral_words = len(words) - moral_words
        try:
...
def score_mfd2(doc):
    ...
    moral_words = [mfd2[token]["foundation"] for token in words if token in mfd2.keys()]
    ...
    doc.user_data["score"] = mfd2_score
    return doc
        ...
        with progressbar.ProgressBar(max_value=num_docs, widgets=widgets) as bar:
            ...
            for i, row in csv.iterrows():
                if row["word"] in emfd.keys():
                    moral_words.append(
                        {"scores": emfd[row["word"]], "freq": row["freq"]}
                    )
            ...
            for dic in moral_words:
                ...
                emfd_score["fairness_sent"] += (
                    dic["scores"]["fairness_sent"] * dic["freq"]
                )
                emfd_score["loyalty_sent"] += (
                    dic["scores"]["loyalty_sent"] * dic["freq"]
                )
                emfd_score["authority_sent"] += (
                    dic["scores"]["authority_sent"] * dic["freq"]
                )
                emfd_score["sanctity_sent"] += (
                    dic["scores"]["sanctity_sent"] * dic["freq"]
                )
                bar.update(i)
...
def spacy_ner(doc):
    ...
    entities = {
        ent.text: ent.text.split(" ") for ent in doc.ents if ent.label_ in include_ents
    }
    cc_processed = {
        e: {
            "patient_words": [],
        ...
        if token not in stopwords:
            if token.dep_ == "nsubj" or token.dep_ == "ROOT":
                ...
                if word in emfd.keys():
                    try:
                        cc_processed[find_ent(token.text, entities)][
                            "agent_words"
                        ].append(word)
                        cc_processed[find_ent(token.text, entities)][
                            "agent_scores"
            ...
            if token.dep_ == "prep":
                ...
                if word in emfd.keys():
                    for child in token.children:
                        try:
                            cc_processed[find_ent(str(child), entities)][
                                "patient_words"
                            ].append(word)
                            cc_processed[find_ent(str(child), entities)][
                                "patient_scores"
            ...
            if token.text == "is":
                try:
                    ...
                    if word in emfd.keys():
                        cc_processed[find_ent(str(children[0]), entities)][
                            "attribute_words"
                        ].append(word)
                        cc_processed[find_ent(str(children[0]), entities)][
                            "attribute_scores"
            ...
            if token.dep_ == "conj":
                if (
                    str(doc[token.right_edge.i]) == "."
                    or str(doc[token.right_edge.i]) == "!"
                    or str(doc[token.right_edge.i]) == "?"
                ):
                    word = token.head.text.lower()
                    if word in emfd.keys():
                        try:
                            cc_processed[
                                find_ent(str(doc[token.right_edge.i - 1]), entities)
                            ]["agent_words"].append(word)
                            cc_processed[
                                find_ent(str(doc[token.right_edge.i - 1]), entities)
                            ]["agent_scores"].append(emfd[word])
                        except:
                            pass
                else:
                    word = token.head.text.lower()
                    if word in emfd.keys():
                        try:
                            cc_processed[find_ent(str(token.right_edge), entities)][
                                "agent_words"
                            ].append(word)
                            cc_processed[find_ent(str(token.right_edge), entities)][
                                "agent_scores"
...
def pat_docs(csv, num_docs):
    ...
    att_scores = [
        c for c in df.columns if c.startswith("attribute") and c.endswith("p")
    ]
    att_sent = [
        c for c in df.columns if c.startswith("attribute") and c.endswith("sent")
    ]

    return df[
        ["NER"] + words + a_mf + a_sent + p_scores + p_sent + att_scores + att_sent
    ].sort_values("NER")
